...
export default ({rovers, mini, handler}) => {
  ...
  const displayCards = rovers.map(rover=>(
    <div className="column" key={rover.id}>
      <a onClick={()=>handler(rover)}>
        <div className="card"
          key={rover.id}
          style={{...style,  backgroundImage: `linear-gradient(to bottom, rgba(0,0,0,.9) 0%, rgba(0,0,0,0.3) 50%, rgba(0,0,0,0.7) 100%), url(${rover.bg})`}}>
  ...
  return (
    <section className="section" style={style.slideshow}>
